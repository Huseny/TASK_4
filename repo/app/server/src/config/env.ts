...
const EnvSchema = z.object({
  MS_PORT: z.coerce.number().int().positive().default(3000),
  MS_HOST: z.string().default("127.0.0.1"),
  MS_CLIENT_ORIGIN: z.string().default("http://127.0.0.1:3000"),
  ...
  MS_REFRESH_TOKEN_TTL_SECONDS: z.coerce
    .number()
    .int()
    .positive()
    .default(28_800),
  ...
  MS_RUNS_RETENTION_PER_PROJECT: z.coerce
    .number()
    .int()
    .positive()
    .default(500),
  MS_NOTIFICATIONS_RETENTION_PER_USER: z.coerce
    .number()
    .int()
    .positive()
    .default(200),
...
export function loadEnv(): EnvVars {
  const defaults: Record<string, string> = {
    MS_JWT_ACCESS_SECRET:
      process.env.NODE_ENV === "test"
        ? "test-access-secret-test-access-secret-32bytes"
        : "",
    MS_JWT_REFRESH_SECRET:
      process.env.NODE_ENV === "test"
        ? "test-refresh-secret-test-refresh-secret-32bytes"
        : "",
  };
...
export function toAppConfig(v: EnvVars): AppConfig {
  return {
    http: {
      host: v.MS_HOST,
      port: v.MS_PORT,
      clientOrigin: v.MS_CLIENT_ORIGIN,
    },
    ...
    pipeline: {
      allowedRepoRoots: splitList(v.MS_ALLOWED_REPO_ROOTS).map((p) =>
        path.resolve(p),
      ),
